...
@export var auto_start: bool = true ## Start Moving on scene start.
...
func _move(delta):
	...
	if direction == Vector2.ZERO or speed == 0.0 or not auto_start:
		return
	if towards_end:
...
func _on_start_moving_area_body_entered(body:Node2D):
	if body.name == "Player":
		auto_start = true
